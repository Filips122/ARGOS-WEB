...
from lab_alerts_features import (
    FEATURE_COLUMNS,
    FEATURE_PROFILES,
    LABEL_PROXY_EXCLUSIONS,
    EXTRACTOR_VERSION,
    feature_columns_for_profile,
    feature_profile_payload,
...
DEFAULT_IN_FILE = Path("LAB_ALERTS_REFACTOR/raw_source/alerts.json")
DEFAULT_OUT_DIR = Path("LAB_ALERTS_REFACTOR/prepared_dataset")
...
def unique_nonempty_count(series: pd.Series) -> int:
...
def write_dataset_metadata(base_dir: Path, df: pd.DataFrame, maps: Dict[str, Dict[str, int]], args: argparse.Namespace, split_mode: str, split_indices: Dict[str, np.ndarray], rare_classes: Dict[str, int], feature_columns: Sequence[str]) -> None:
    ...
    contract = schema_contract_payload(
    ...
    write_json(base_dir / "schema_contract.json", contract)
    write_json(
        base_dir / "prepare_dataset_summary.json",
        {
            ...
            "schema_contract_path": str(base_dir / "schema_contract.json"),
