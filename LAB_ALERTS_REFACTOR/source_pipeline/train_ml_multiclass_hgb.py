...
def run_one(split_mode: str, fold: int | None, sample_frac: float | None, epochs: int, out_dir, dataset: str, datasets_base: str) -> dict:
    ...
    X_train_df = pd.DataFrame(X_train, columns=train.feature_names)
    X_val_df = pd.DataFrame(X_val, columns=train.feature_names)
    X_test_df = pd.DataFrame(X_test, columns=train.feature_names)
    ...
    save_staged_classification_history(out_dir, model, X_train_df, y_train, X_val_df, y_val, encoder.classes_)
    p_train = model.predict_proba(X_train_df)
    p_val = model.predict_proba(X_val_df)
    p_test = model.predict_proba(X_test_df)
    ...
    save_json(out_dir / "model_feature_names.json", {"feature_names_in": list(map(str, getattr(model, "feature_names_in_", train.feature_names)))})
...
def main() -> None:
    ...
    root = artifacts_root("offline_LAB_ALERTS_multiclass_hgb", args.split_mode, now_run_id(), dataset=args.dataset, datasets_base=args.datasets_base, run_config={"sample_frac": args.sample_frac, "epochs": args.epochs, "datasets_base": args.datasets_base})
    if args.split_mode != "groupkfold":
        try:
            out = run_one(args.split_mode, None, args.sample_frac, args.epochs, root, args.dataset, args.datasets_base)
    ...
    for fold in folds:
        ...
        try:
            out = run_one("groupkfold", int(fold), args.sample_frac, args.epochs, fold_dir, args.dataset, args.datasets_base)
        except (FileNotFoundError, EmptySplitError) as exc:
